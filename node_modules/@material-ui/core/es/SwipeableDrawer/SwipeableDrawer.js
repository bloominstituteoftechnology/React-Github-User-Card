...
const SwipeableDrawer = React.forwardRef(function SwipeableDrawer(props, ref) {
  ...
  const openRef = React.useRef(open); // Use a ref so the open value used is always up to date inside useCallback.
  ...
  const handleBodyTouchEnd = React.useCallback(event => {
  ...
  const handleBodyTouchMove = React.useCallback(event => {
  ...
  const handleBodyTouchStart = React.useCallback(event => {
